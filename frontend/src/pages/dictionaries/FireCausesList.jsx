...
import { Loader2, Plus, Search, Edit, Trash2, Flame } from 'lucide-react';

const FireCausesList = () => {
    const { has } = usePermissions();
    const { data: items, isLoading, error } = useFireCauses();
    const create = useCreateFireCause();
    const update = useUpdateFireCause();
    const del = useDeleteFireCause();

    const canManage = has('dictionaries.manage');

    const [search, setSearch] = useState('');
    const [editing, setEditing] = useState(null); // null → не редактируем
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-2xl font-bold text-slate-800">Причины пожара</h1>
                </div>
                {canManage && (
                    <Button onClick={openNew} className="rounded-lg bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700">
                        <Plus className="h-4 w-4 mr-2" /> Добавить причину
                    </Button>
                )}
            </div>
            ...
            {editing && (
                <div className="rounded-xl border border-slate-200 p-4">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="cause-name">Название</Label>
                        <Input id="cause-name" value={name} onChange={(e) => setName(e.target.value)} className="rounded-lg" />
                    </div>
            ...
            <AlertDialog open={!!toDelete} onOpenChange={(o) => !o && setToDelete(null)}>
                <AlertDialogContent className="rounded-2xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Удалить причину?</AlertDialogTitle>
                        <AlertDialogDescription>Причина «{toDelete?.name}» будет удалена навсегда. Это действие нельзя отменить.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
